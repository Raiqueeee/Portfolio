...
export function CodingStatsPanel() {
    ...
    const { weeks, monthLabels, totalGH, totalLC } = useMemo(() => {
        ...
        if (!ghData?.contributions?.length) {
            const activeDays = new Set([3, 9, 14, 22, 31, 37, 46, 58, 66, 73, 81, 94, 101, 108, 119, 127, 136, 145, 153, 161, 174, 182, 191, 199, 207, 219, 228, 236, 244, 251, 263]);
            allDays.forEach((day, index) => {
                const cycleIndex = index % 273;
                const active = activeDays.has(cycleIndex);
                const level = active ? (index % 4) + 1 : 0;
                ghMap[formatISO(day, { representation: "date" })] = { count: active ? level : 0, level };
            });
        }
